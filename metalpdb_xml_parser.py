...
"""
MetalPDB XML Parser
CNRS UMR 9198 I2BC - IMAPP Team
jean-charles.carvaillo@i2bc.paris-saclay.fr
"""
...
def parsing_arguments():
    ...
    """
    parser = argparse.ArgumentParser(usage="python3 %(prog)s [-h] [-i]" +
                                     "[metalPDB_xml_file_path]\n",
                                     description="Method to parse metalPDB\
                                     xml database and to performed\
                                     statistics stuff")

    parser.add_argument("-i", "--input",
                        help="path of the xml file which contain \
                        metalPDB database",
                        default=False,
                        nargs=1,
                        type=str,
    ...
    parser.add_argument("-m", "--metal",
                        help="allow to filter database by metal (metal pdb \
                        name) specified",
                        default=False,
                        nargs=1,
                        type=str,
    ...
    if args.metal:
        if args.metal[0] not in METAL:
            exit("ERROR : The metal specified doesn't exist.\n")
    else:
        args.metal = [""]

    return (args.input[0], args.metal[0])


def parsing_metal_pdb_xml(filename, metal_query):
    """
    Method to parse metalPDB xml database file by parsing process.

    Return:
        sites (list): a list which contains metal binding sites parsed
...
def stat_HIS_protonate_state(metal_query, sites):
    """
    Method to count total number histidine residue in function of his
        protonate state.
    ...
    for site in sites:
        ligands = site.getMetal().getLigands()
        for ligand in ligands:
            if "HIS" in ligand.getResidueName():
                cpt_HIS += 1

                if "E" in ligand.getDonor().getAtomName() and \
                   "Side chain" in ligand.getDonor().getInteraction():
                    cpt_HID += 1

                if "D" in ligand.getDonor().getAtomName() and \
                   "Side chain" in ligand.getDonor().getInteraction():
                    cpt_HIE += 1

    print("total number of {} sites: {}".format(metal_query, len(sites)))
    print("total number of Histidine: {}".format(cpt_HIS))
    print("total number of Histidine protonate on epsilon: {}".format(cpt_HIE))
    print("total number of Histidine protonate on delta: {}".format(cpt_HID))
...
if __name__ == "__main__":
    ...
    metal_query = arguments[1]

    start_time = time()

    print("\n*****************************")
    print("* PARSING METALPDB XML FILE *")
    print("*****************************\n")

    SITES = parsing_metal_pdb_xml(xml_file, metal_query)

    print("\n*************************************")
    print("* HISTIDINE PROTONATE STATE RESULTS *")
    print("*************************************\n")

    stat_HIS_protonate_state(metal_query, SITES)
    ...
    if _platform == "linux" or _platform == "linux2":
        print("Maximum RAM: {0:13.2f} Mo".format(ressources[2] / 1024.0))
